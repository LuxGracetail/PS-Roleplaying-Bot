...
//		var text = config.excepts.indexOf(toId(by)) < 0 ? '/pm ' + by + ', ' : '';
	...
	start: function(arg, by, room) {
		...
		if (/conquest/i.test(toId(this.RP[room].plot))) {
				...
				this.say(room, '**Arceus, Yveltal, Kyogre and Zygarde 100% are banned. Warlords can have one legendary OR one mega and one RU legend. They may have up to two knights and only three kingdoms are allowed in an alliance. Battle in the Ubers format. Kingdoms can accept battles once every FIVE minutes.**');
				this.say(room, "__All participants may have one mega and one RU (or below) legend . Meloetta, Mega Kanga, Gengar, Mawile, Lucario, Slowbro, Salamence, Altaria, Blaziken, Metagross, Sableye, Aegislash (for Steel ONLY), any kind of Greninja, Charizard-X, and count as a legendary spot.__");
				...
				this.say(room, "**As of now, all Ultrabeasts, including Pheromosa are legend slots, i.e. lord only.  Z moves are allowed.**");
