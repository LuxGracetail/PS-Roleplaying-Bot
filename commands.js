...
//		var text = config.excepts.indexOf(toId(by)) < 0 ? '/pm ' + by + ', ' : '';
	...
	start: function(arg, by, room) {
		...
		if (/conquest/i.test(toId(this.RP[room].plot))) {
				...
				this.say(room, "**If you attack a kingdom and lose to the knight, you cannot re-challenge until another kingdom that isn’t part of your alliance attacks. The Evasion/BP Clause is in effect, and Geomancy, Soul Dew, Smooth Rock, and Damp Rock are banned. No challenges in the first fifteen minutes.**");
