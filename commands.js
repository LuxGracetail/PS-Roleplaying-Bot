...
//		var text = config.excepts.indexOf(toId(by)) < 0 ? '/pm ' + by + ', ' : '';
	...
	start: function(arg, by, room) {
		...
		if (/conquest/i.test(toId(this.RP[room].plot))) {
				...
				this.say(room, '**Arceus, Yveltal, Kyogre, Zygarde 100% and Ash Greninja are banned. Warlords can have one legendary OR one mega and one RU legend. They may have up to two knights and only three kingdoms are allowed in an alliance. Battle in the Ubers format. Kingdoms can accept battles once every FIVE minutes.**');
				this.say(room, "__All participants may have one mega and one RU (or below) legend . Meloetta, Mega Kanga, Gengar, Mawile, Lucario, Slowbro, Salamence, Altaria, Blaziken, Metagross, Sableye, Aegislash (for Steel ONLY), Greninja, Charizard-X, and count as a legendary spot.__");
				...
				this.say(room, "**If you attack a kingdom and lose to the knight, you cannot re-challenge until another kingdom that isn’t part of your alliance attacks. The Evasion/BP Clause is in effect, and Geomancy, Smooth Rock, and Damp Rock are banned. No challenges in the first fifteen minutes.**");
				...
				this.say(room, "**As of now, all Ultrabeasts, with the exception of Pheromosa, which has been temporarily quickbanned, and will undergo further testing later, are legend slots, i.e. lord only.  Z moves are allowed.**");
				this.say(room, "__For RPing, legendary bans do not strictly apply - you can RP with most legendaries, including Kyogre and Yveltal.  The following exceptions apply: Primals, Arceus, banned Mega Legends and Ash Greninja.__");
